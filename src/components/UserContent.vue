<template>
    <v-row align="center">
        <v-col md="1">
            <v-avatar color="#990033" :size="avatarSize"><img src="data:image/png;base64,"/></v-avatar>
        </v-col>
        <v-col md="2" class="text-center">
            {{ user.name }}
        </v-col>
        <v-col md="4" class="text-center">
           {{ user.email }}
        </v-col>
        <v-col md="2"> 
            {{ getUserStatus(user) }}
        </v-col>
        <v-col md="2">
            {{ getUserAuthority(user) }}
        </v-col>
        <v-col md="1">
            <v-btn icon small class="ml-2">
                <v-icon color="yellow darken-3">
                    mdi-pencil
                </v-icon>
            </v-btn>
            <v-btn icon small class="ml-2">
                <v-icon color="red darken-3">
                    mdi-delete
                </v-icon>
            </v-btn>
        </v-col>
    </v-row>
</template>
<script>
export default {
    props:["user","avatarSize"],
    name: 'UserContent',
    methods: {
        getUserStatus(user) {
            if (user.enabled && user.accountNonExpired && user.credentialsNonExpired && user.nonLocked) {
                return "ACTIVE";
            } else if (!user.enabled) {
                return "DISABLED";
            } else if (!user.accountNonExpired || !user.credentialsNonExpired) {
                return "EXPIRED";
            } else if (!user.nonLocked) {
                return "BLOCKED";
            } else {
                return "UNKNOWN";
            }
        },
        getUserAuthority(user) {
            return user.authorities[0] != null ? user.authorities[0].authority.replace("ROLE_","") : "UNKOWN";
        }
    }
}
</script>